...
/** Extract user-visible text from a DSH content-block array (or plain string). */
...
/**
 * Extract a plain title string from a readTitle() return value.
 * DSH's sessionQuery.readTitle() returns a title *snapshot object*
 * ({title, messageSeqs, source, eventSeq, updatedAt}), not a bare string.
 * Without this, /list shows "[object Object]" instead of the session name.
 */
...
// ------------------------------------------------------ markdown → Telegram
// Telegram has no tables, so markdown tables are re-rendered as aligned
// fixed-width text inside <pre> (East-Asian display width aware), and common
// inline markdown (**bold**, `code`) maps to native Telegram entities.

/** Terminal display width of one string; CJK/fullwidth characters count as 2. */
...
/** Clip to a display-width budget, appending … when something was cut. */
...
/** Pad with spaces up to a display-width budget. */
function padToWidth(text, width) {
	return text + " ".repeat(Math.max(0, width - displayWidth(text)));
}

/** Strip common inline markdown so table cells stay readable in <pre>. */
...
/**
 * Convert assistant markdown to Telegram HTML: markdown tables become
 * column-aligned pre blocks, bold and inline-code markdown become native entities,
 * everything else is escaped as before.
 */
function telegramifyMarkdown(text) {
	const lines = String(text ?? "").split("\n");
	const tableLine = /^\s*\|.*\|\s*$/;
	const separatorLine = /^\s*\|?[\s:|-]+\|?\s*$/;
	const out = [];
	let i = 0;
	while (i < lines.length) {
		const isTable = tableLine.test(lines[i])
			&& i + 1 < lines.length
			&& tableLine.test(lines[i + 1])
			&& separatorLine.test(lines[i + 1]);
		if (!isTable) {
		...
		const rows = [];
		while (i < lines.length && tableLine.test(lines[i])) {
			if (!separatorLine.test(lines[i])) {
				const inner = lines[i].trim().replace(/^\|/, "").replace(/\|$/, "");
				rows.push(inner.split("|").map((cell) => stripInlineMarkdown(cell.trim())));
			}
			i += 1;
		}
		if (rows.length > 0) {
			...
			for (let c = 0; c < cols; c++) {
				widths.push(Math.min(maxWidth, Math.max(1, ...rows.map((row) => displayWidth(row[c] ?? "")))));
			}
			const rendered = rows.map((row) => {
				const cells = [];
				for (let c = 0; c < cols; c++) {
					cells.push(padToWidth(clipToWidth(row[c] ?? "", widths[c]), widths[c]));
				}
				return cells.join(" │ ").trimEnd();
			});
			const separator = widths.map((width) => "─".repeat(width)).join("─┼─");
			out.push(`<pre>${esc([rendered[0], separator, ...rendered.slice(1)].join("\n"))}</pre>`);
		}
	}
	return out.join("\n");
}
...
/**
 * Dedupe key linking the approval waterfall request (toolName/callId/reason)
 * to its durable `approval/asked` session event, which carries the same fields
 * plus the log id. callId identifies a tool call uniquely when present.
 */
function approvalKey(sessionId, toolName, callId, reason) {
	return `${sessionId}|${callId ?? "-"}|${toolName ?? "?"}|${truncate(reason ?? "", 80)}`;
}

/** Human phrasing for the closed outcomes logged in `approval/decided`. */
function approvalOutcomeMark(outcome) {
	switch (outcome) {
		case "allowed-once": return "✅ 已允许";
		case "rejected": return "⛔ 已拒绝";
		case "cancelled": return "⏹ 已取消";
		default: return `⚠️ 无人处理（${outcome ?? "unknown"}）`;
	}
}
...
class TelegramBridge {
	constructor(ctx, config, token) {
		...
		this.approvalKeys = new Map(); // approvalKey -> pendingId (the waterfall card we own)
		this.approvalNotices = new Map(); // approval/asked id -> {chatId, messageId} (read-only desktop notices)
		this.approvalWaits = new Map(); // approvalKey -> timer (grace before the read-only notice)
		this.approvalOwned = new WeakSet(); // agents whose scoped ctx carries our approval/question listeners
	...
	loadState() {
		try {
			if (existsSync(STATE_PATH)) {
				...
				// `chats` is an OBJECT keyed by chat id (see chatRecord). The old
				// check demanded an Array, which is never what saveState writes —
				// every restart silently discarded the file and wiped all
				// chat↔session bindings: approvals stopped reaching Telegram and
				// /open came back to a session with no memory of its links.
		...
		return { version: 1, chats: {}, sessions: {}, tags: {} };
	}
	...
	async start() {
		...
		this.disposeEvents = [
			...
			// Own every live agent's approval/question waterfall at the AGENT
			// scope: the dispatch chain walks agent -> session -> root, so a
			// listener on agent.ctx answers BEFORE the desktop UI's
			// session-scoped one — that is what makes the Telegram buttons
			// work while the session is open on the desktop.
			this.ctx.on("agent/created", (payload) => this.installApprovalOwner(payload?.agent))
		];
	...
	stop() {
		...
		this.pending.clear();
		for (const timer of this.approvalWaits.values()) clearTimeout(timer);
	...
	async api(method, params = {}) {
		...
		try {
			res = await this.rawRequest(url, "POST", {
				...
				"User-Agent": "dsh-plugin-telegram/0.3.0"
			}, body, timeoutMs);
		} catch (error) {
	...
	async registerCommands() {
		await this.api("setMyCommands", {
			commands: [
				...
				{ command: "ws", description: "查看/切换工作区" },
	...
	async onCommand(chatId, text, message) {
		...
		switch (cmdName) {
			...
			case "/new": {
				if (!args) {
					await this.send(chatId, "用法：<code>/new 任务描述</code>\n例如：<code>/new 分析一下当前目录的代码结构</code>\n指定工作区：<code>/new @E:\\project 分析代码</code>");
					return;
				}
				// /new @<path> <prompt> — create in a specific workspace
				let wsOverride;
				let prompt = args;
				if (args.startsWith("@")) {
					const spaceIdx = args.indexOf(" ");
					if (spaceIdx > 1) {
						wsOverride = args.slice(1, spaceIdx);
						prompt = args.slice(spaceIdx + 1).trim();
					}
				}
				if (!prompt) {
					await this.send(chatId, "用法：<code>/new @工作区路径 任务描述</code>\n例如：<code>/new @E:\\dsh\\myapp 修复登录bug</code>");
					return;
				}
				await this.createSession(chatId, prompt, wsOverride);
				return;
			}
			case "/ask": {
			...
			case "/cancel": {
				// Only the CURRENT session (explicit id allowed). Other projects'
				// running turns are never touched — the user asked for exactly
				// that; cross-session sweeping surprised them.
				const record = this.chatRecord(chatId);
				...
				if (agent === undefined || agent.status !== "running") {
					await this.send(chatId, `${this.sessionLabel(sessionId)} 当前没有正在运行的回合。`);
					return;
				}
				try {
					agent.cancel({ kind: "user" }, { keepInbox: true });
					this.ctx.logger.info(`dsh-plugin-telegram: /cancel requested for agent ${sessionId}`);
					await this.send(chatId, `⏹ 已请求取消回合：${this.sessionLabel(sessionId)}`);
				} catch (error) {
					this.ctx.logger.warn(`dsh-plugin-telegram: /cancel failed for ${sessionId}: ${error instanceof Error ? error.message : error}`);
			...
			case "/workspace": {
				const record = this.chatRecord(chatId);
				if (!args) {
					const current = record.workspace || this.cfg.workspace || "(home)";
					await this.send(chatId, `📁 当前工作区：<code>${esc(current)}</code>\n\n切换：<code>/ws E:\\project\\path</code>\n下次 <code>/new</code> 会在这个目录创建会话。\n临时指定：<code>/new @E:\\path 任务</code>`);
					return;
				}
				try {
					mkdirSync(args, { recursive: true });
					record.workspace = args;
					this.scheduleSave();
					await this.send(chatId, `✅ 工作区已切换：<code>${esc(args)}</code>\n下次 <code>/new</code> 会在这个目录创建会话。`);
				} catch (error) {
					await this.send(chatId, `切换工作区失败：${esc(error instanceof Error ? error.message : error)}`);
				}
				return;
			}
			case "/ping":
	...
	async sendHelp(chatId) {
		const lines = [
			...
			"<code>/new @路径 任务</code> — 在指定工作区新建",
			"<code>/ws [路径]</code> — 查看/切换工作区",
	...
	async listSessions(chatId, limit) {
		try {
			...
			for (let i = 0; i < top.length; i++) {
				...
				const title = titles[i]?.status === "fulfilled" ? titleText(titles[i].value) : "";
				...
				lines.push(`<code>${i + 1}</code>. <b>${esc(this.displaySessionName(id, title))}</b> ${current ? "📍" : ""}`);
	...
	/**
	 * Human display name for a session list row: real title first; sessions
	 * whose title is missing (or is just the bare `session-uuid`) fall back to
	 * the remembered project tag, then to a shortened id.
	 */
	displaySessionName(id, title) {
		const clean = String(title ?? "").trim();
		if (clean !== "" && !/^session-[0-9a-f-]{36}$/i.test(clean)) return clean;
		const tag = this.state.tags[id];
		if (tag !== undefined) return `「${tag}」`;
		return id.length > 21 ? `${id.slice(0, 21)}…` : id;
	}

	async openSession(chatId, sessionId) {
		try {
			const title = titleText(await this.ctx.sessionQuery.readTitle(sessionId));
			this.linkSession(chatId, sessionId);
			this.rememberTag(sessionId, title);
			await this.send(chatId, `📍 当前会话已切换：<b>${esc(this.displaySessionName(sessionId, title))}</b>\n<code>${esc(sessionId)}</code>\n现在可以直接发消息继续对话。`);
			// restore context: the user may be coming back to a project after
			// days on another one — surface what this session "already has".
			await this.statusSession(chatId, sessionId);
			await this.tailSession(chatId, sessionId, 3);
		} catch (error) {
	...
	/** Persist a human-readable short tag for a session (push labels, recaps). */
	rememberTag(sessionId, title) {
		const tag = String(title ?? "").trim().replace(/\s+/g, " ");
		if (tag === "" || this.state.tags[sessionId] !== undefined) return;
		this.state.tags[sessionId] = truncate(tag, 24);
		this.scheduleSave();
	}

	/** Short label for a session: its remembered tag or a short id fallback. */
	sessionTag(sessionId) {
		return String(this.state.tags[sessionId] ?? sessionId.slice(0, 6));
	}

	/**
	 * Ready-to-embed HTML label for a session: the remembered project tag when
	 * known (cards stop showing raw `session-uuid` code), else the full id.
	 */
	sessionLabel(sessionId) {
		const tag = sessionId !== undefined ? this.state.tags[sessionId] : undefined;
		return tag !== undefined ? `「${esc(tag)}」` : `<code>${esc(sessionId ?? "?")}</code>`;
	}

	/**
	 * Prefix for event pushes from a session. Only annotate when the chat has
	 * more than one linked session — with a single project the extra noise of
	 * a tag on every message is not worth it.
	 */
	pushTag(chatId, sessionId) {
		const linked = this.state.chats[String(chatId)]?.linked?.length ?? 0;
		if (linked <= 1) return "";
		return `「${esc(this.sessionTag(sessionId))}」`;
	}
	...
	/**
	 * Chat that should receive actionable requests (approvals / questions) for
	 * sessions with no linked chat — e.g. sessions created in the desktop UI.
	 * Without this, onApproval silently falls through to next() and the request
	 * never reaches the phone. Deterministic on purpose: the first allowed chat,
	 * or the single auto-authorized chat; undefined when ambiguous (multiple
	 * auto-authorized chats) so we never guess where a permission grant goes.
	 */
	fallbackChat() {
		if (this.cfg.allowedChats.length > 0) return this.cfg.allowedChats[0];
		if (this.cfg.autoAuthorize) {
			const ids = Object.keys(this.state.chats);
	...
	/**
	 * Resolve the configured agent preset, falling back to "standard" when the
	 * configured id does not exist in this DSH install (e.g. "default" — not
	 * a preset here, which made every /new fail with "preset not found").
	 */
	async resolveAgentPreset() {
		const wanted = this.cfg.agentPreset ?? "standard";
		try {
			await this.ctx.agentPresets.resolve(wanted);
			await this.ctx.agentPresets.standingKeyFor(wanted);
			return wanted;
		} catch (error) {
			const fallback = "standard";
			this.ctx.logger.warn(`dsh-plugin-telegram: agent preset "${wanted}" unavailable (${error instanceof Error ? error.message : error}); falling back to "${fallback}"`);
			await this.ctx.agentPresets.resolve(fallback);
			await this.ctx.agentPresets.standingKeyFor(fallback);
			return fallback;
		}
	}

	async createSession(chatId, prompt, workspaceOverride) {
		try {
			// workspace priority: explicit override (@path in /new) > per-chat
			// choice (/ws) > config.workspace > home dir
			const record = this.chatRecord(chatId);
			const workspacePath = (workspaceOverride ?? record.workspace ?? this.cfg.workspace ?? "").trim() || os.homedir();
			...
			this.linkSession(chatId, sessionId);
			this.rememberTag(sessionId, titleFromPrompt(prompt));
	...
	async tailSession(chatId, sessionId, n) {
		try {
			this.linkSession(chatId, sessionId);
			const seen = [];
			// listEvents returns LIGHTWEIGHT records (no `data`), so message text
			// must come from readSurface — the folded current conversation with
			// full payloads. The listEvents path stays only as a fallback.
			...
			try {
				...
				for (const event of surface.events) {
					...
					const text = event.type === "user/message"
						? contentText(event.data?.content ?? event.data?.message?.content)
						: contentText(event.data?.message?.content ?? event.data?.content);
					if (!text) continue;
					seen.push({ type: event.type, text });
				}
			} catch (surfaceError) {
				this.ctx.logger.warn(`dsh-plugin-telegram: readSurface unavailable, falling back to listEvents: ${surfaceError instanceof Error ? surfaceError.message : surfaceError}`);
			}
			if (!usedSurface) {
			...
			const lines = last.map((item) => `${item.type === "user/message" ? "👤" : "🤖"} ${telegramifyMarkdown(truncate(item.text, 1200))}`);
			await this.send(chatId, `📜 <b>最近 ${last.length} 条</b>（${this.sessionLabel(sessionId)}）\n\n${lines.join("\n\n")}`);
		} catch (error) {
	...
	async statusSession(chatId, sessionId) {
		try {
			this.linkSession(chatId, sessionId);
			const agent = this.ctx.agents.get(sessionId);
			const title = titleText(await this.ctx.sessionQuery.readTitle(sessionId).catch(() => ""));
			const events = await this.ctx.sessionQuery.listEvents(sessionId);
			// listEvents records carry no `data`; todos come from filterEvents,
			// which scans full semantic documents
			let todos = [];
			try {
				const docs = await this.ctx.sessionQuery.filterEvents(sessionId, [{ kind: "type", values: ["todo/write"] }]);
				for (const doc of docs) {
					const written = doc.data?.todos;
					if (Array.isArray(written)) todos = written;
				}
			} catch (error) {
				this.ctx.logger.warn(`dsh-plugin-telegram: todo scan failed: ${error instanceof Error ? error.message : error}`);
			}
			const lines = [
				`📊 <b>会话状态</b>`,
				esc(this.sessionLabel(sessionId)),
				title ? `<b>${esc(title)}</b>` : "",
	...
	onSessionEvent(session, event) {
		...
		if (chatId === undefined) return;
		const tag = this.pushTag(chatId, sessionId);
		try {
			switch (event.type) {
				case "user/message": {
				...
				case "assistant/message": {
					...
					if (!text) break;
					// truncate the RAW text first: cutting the converted HTML
					// could split a <pre>/<b> tag and fail Telegram's parser
					this.send(chatId, `${tag}🤖 ${telegramifyMarkdown(truncate(text, this.cfg.maxMessageLength))}`).catch(() => {});
					break;
				}
				case "turn/start": {
					...
					this.turnMessages.set(sessionId, { chatId, messageId: null, tag });
					this.send(chatId, `${tag}⏳ DSH 正在处理…`).then((sent) => {
				...
				case "tool/call": {
					...
					this.send(chatId, `${tag}🔧 <code>${esc(name2)}</code> ${snippet ? `\n<code>${esc(snippet)}</code>` : ""}`).catch(() => {});
					break;
				}
				case "todo/write": {
				...
				case "approval/asked": {
					// Durable audit event fired BEFORE the approval waterfall runs.
					// When the session is open on the desktop, the desktop UI's
					// session-scoped listener answers first and the plugin's
					// waterfall handler never runs — without this observer the
					// phone would never learn about the request. Wait briefly:
					// if our own interactive card went out (approvalKeys), skip;
					// otherwise push a read-only notice and sync the outcome.
					if (!this.cfg.forwardApprovals) break;
					const key = approvalKey(sessionId, event.data?.toolName, event.data?.callId, event.data?.reason);
					const askedId = event.data?.id;
					const wait = setTimeout(() => {
						this.approvalWaits.delete(key);
						if (this.approvalKeys.has(key)) return;
						this.notifyDesktopApproval(chatId, sessionId, askedId, event.data, tag).catch((error) => {
							this.ctx.logger.warn(`dsh-plugin-telegram: desktop approval notice failed: ${error instanceof Error ? error.message : error}`);
						});
					}, 2000);
					this.approvalWaits.set(key, wait);
					break;
				}
				case "approval/decided": {
					const askedId = event.data?.id;
					const notice = this.approvalNotices.get(askedId);
					if (notice === undefined) break;
					this.approvalNotices.delete(askedId);
					this.edit(notice.chatId, notice.messageId, notice.baseText + `\n\n— ${approvalOutcomeMark(event.data?.outcome)}`).catch(() => {});
					break;
				}
				default:
					break;
	...
	/**
	 * Register our approval/question listeners on one agent's OWN scope, so we
	 * answer before the desktop UI (session scope) in the approval waterfall.
	 * Safe to call repeatedly; skips agents without a scoped context.
	 */
	installApprovalOwner(agent) {
		const agentCtx = agent?.ctx;
		if (agentCtx === undefined || typeof agentCtx.on !== "function") {
			this.ctx.logger.warn(`dsh-plugin-telegram: agent ${agent?.id ?? "?"} has no scoped ctx — approvals stay on the global listener`);
			return;
		}
		if (this.approvalOwned.has(agent)) return;
		this.approvalOwned.add(agent);
		try {
			// `true` = prepend: the flat listener list runs in registration order
			// and the desktop UI's session-scoped listener was registered when
			// the client connected — before us. Without prepend the desktop
			// always answers first and the Telegram buttons never appear.
			agentCtx.on("approval/request", (request, next) => this.onApproval(request, next), true);
			agentCtx.on("user-questions/request", (request, next) => this.onQuestion(request, next), true);
			this.ctx.logger.info(`dsh-plugin-telegram: approval owner installed on agent ${agent.id ?? "?"}`);
		} catch (error) {
			this.approvalOwned.delete(agent);
			this.ctx.logger.warn(`dsh-plugin-telegram: cannot own approvals for agent ${agent.id ?? "?"}: ${error instanceof Error ? error.message : error}`);
		}
	}

	/** Attach to agents that are already live when the bridge (re)starts. */
	adoptLiveAgents() {
		try {
			let count = 0;
			for (const entry of this.ctx.agents.store.values()) {
				this.installApprovalOwner(entry.agent);
				count += 1;
			}
			this.ctx.logger.info(`dsh-plugin-telegram: swept ${count} live agent(s) at startup`);
		} catch (error) {
			this.ctx.logger.warn(`dsh-plugin-telegram: cannot sweep live agents: ${error instanceof Error ? error.message : error}`);
		}
	}
	...
	onApproval(request, next) {
		...
		const key = approvalKey(sessionId, request.toolName, request.callId, request.reason);
		this.ctx.logger.info(`dsh-plugin-telegram: approval waterfall reached the plugin (session ${sessionId ?? "?"}, tool ${request.toolName ?? "?"})`);
		// the same request can reach us twice (agent-scoped listener first, then
		// the global one after a next()) — never present two cards
		if (key && this.approvalKeys.has(key)) return next();
		const linked = sessionId !== undefined ? this.linkedChat(sessionId) : undefined;
		const fallback = linked === undefined;
		const chatId = fallback ? this.fallbackChat() : linked;
		if (chatId === undefined) return next();
		return this.presentApproval(request, chatId, fallback, key, next).catch((error) => {
			this.ctx.logger.warn(`dsh-plugin-telegram: approval push failed (${error instanceof Error ? error.message : error}); falling back to the desktop UI`);
			return next();
		});
	}

	async presentApproval(request, chatId, fallback = false, key = "", next) {
		const sessionId = request.agent.session.id;
		const pendingId = shortId();
		const reason = request.reason ? String(request.reason) : "";
		const lines = [
			"🔐 <b>DSH 请求审批</b>",
			`项目：${this.sessionLabel(sessionId)}`,
			`工具：<code>${esc(request.toolName ?? "?")}</code>`,
			reason ? `原因：${esc(truncate(reason, 500))}` : "",
			fallback ? "\n<i>⚠️ 该会话未绑定此聊天（如桌面端创建的会话），这是一次全局转发。回复 /open 可接管会话。</i>" : "",
			"",
			"允许后仅对本次操作生效。"
		];
		...
		return await new Promise((resolve) => {
			...
			const timer = setTimeout(() => {
				...
				this.pending.delete(pendingId);
				if (key) this.approvalKeys.delete(key);
				this.edit(chatId, messageId, baseText + `\n\n— ⏰ 超时未响应，已转交桌面处理`).catch(() => {});
				// TG had its chance; let the rest of the waterfall (desktop UI) try
				Promise.resolve(typeof next === "function" ? next() : "cancelled").then(resolve, () => resolve("cancelled"));
			}, this.cfg.approvalTimeoutSeconds * 1000);
			pending.timer = timer;
			this.pending.set(pendingId, pending);
			// mark this request as handled by our interactive card so the
			// `approval/asked` read-only notice (fired ~2s later) stays quiet
			if (key) this.approvalKeys.set(key, pendingId);
	...
	/**
	 * Read-only notice for a request the plugin does NOT own (the desktop UI's
	 * session-scoped waterfall listener answers first when the session is open
	 * there, so our interactive card never fires). TG buttons could not resolve
	 * such a request anyway — the outcome lands via `approval/decided`.
	 */
	async notifyDesktopApproval(chatId, sessionId, askedId, data, tag) {
		const reason = data?.reason ? String(data.reason) : "";
		const lines = [
			`${tag}🔐 <b>DSH 请求审批</b>（桌面处理中）`,
			`项目：${this.sessionLabel(sessionId)}`,
			`工具：<code>${esc(data?.toolName ?? "?")}</code>`,
			reason ? `原因：${esc(truncate(reason, 500))}` : "",
			"",
			"<i>该会话正在桌面打开，请在桌面点击「允许 / 拒绝」；决定后这里会同步结果。</i>"
		];
		const baseText = lines.filter(Boolean).join("\n");
		const sent = await this.send(chatId, baseText);
		if (sent?.message_id !== undefined && askedId !== undefined) {
			this.approvalNotices.set(askedId, { chatId, messageId: sent.message_id, baseText });
		}
		this.ctx.logger.info(`dsh-plugin-telegram: desktop-owned approval notice sent (asked id ${askedId ?? "?"})`);
	}
	...
	onQuestion(request, next) {
		...
		const chatId = fallback ? this.fallbackChat() : linked;
		if (chatId === undefined) return next();
		return this.presentQuestion(request, chatId, fallback, next).catch((error) => {
			this.ctx.logger.warn(`dsh-plugin-telegram: question push failed (${error instanceof Error ? error.message : error}); falling back to the desktop UI`);
			return next();
		});
	}

	async presentQuestion(request, chatId, fallback = false, next) {
		const sessionId = request.agent?.session?.id;
		const pendingId = shortId();
		...
		const outcomePromise = new Promise((resolve) => {
			...
			const timer = setTimeout(() => {
				...
				Promise.resolve(typeof next === "function" ? next() : { answers: questions.map((q) => ({ id: q.id, selected: [] })) })
					.then(resolve, () => resolve({ answers: questions.map((q) => ({ id: q.id, selected: [] })) }));
		...
		if (questions.length > 1 || first === undefined) {
			...
			const lines = [
				"❓ <b>DSH 向你提问</b>",
				`项目：${this.sessionLabel(sessionId)}`,
				fallback ? "\n<i>⚠️ 该会话未绑定此聊天（如桌面端创建的会话），这是一次全局转发。</i>" : "",
				"",
